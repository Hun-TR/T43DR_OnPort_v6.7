...
function setupIPInputKeyboardHandlers() {
    ...
    // ✅ Input change olayları — GÜNCELLENMİŞ
    document.addEventListener('input', function(e) {
        if (!e.target.classList.contains('ip-part')) return;
        
        const input = e.target;
        const id = input.id || '';
        const isSecondary = id.startsWith('ntp2-');
        
        // Hangi field'a ait olduğunu tespit et
        let fieldName = null;
        if (id.startsWith('subnet-'))  fieldName = 'subnet';
        if (id.startsWith('gateway-')) fieldName = 'gateway';
        if (id.startsWith('dns-'))     fieldName = 'dns';
        
        setTimeout(() => {
            if (fieldName) {
                // ✅ Subnet / Gateway / DNS güncelle
                updateHiddenIPInputForField(fieldName);
                console.log(`${fieldName} input değişti:`, 
                    document.getElementById(
                        fieldName === 'subnet'  ? 'ntpSubnet'  :
                        fieldName === 'gateway' ? 'ntpGateway' : 'ntpDNS'
                    )?.value
                );
            } else {
                // NTP1 veya NTP2 güncelle
                updateHiddenIPInput(isSecondary);
            }
...
// ✅ YENİ FONKSİYON — hidden input'u günceller ve loglar
function updateNetworkHiddenInput(prefix, hiddenId) {
    const hiddenInput = document.getElementById(hiddenId);
    if (!hiddenInput) return;
    
    const parts = [];
    for (let i = 1; i <= 4; i++) {
        const input = document.getElementById(`${prefix}-part${i}`);
        parts.push(input ? (input.value || '0') : '0');
    }
    
    hiddenInput.value = parts.join('.');
    console.log(`${prefix} güncellendi:`, hiddenInput.value);
    
    // Container validasyonu
    const container = document.getElementById(`${prefix}-part1`)?.closest('.ip-input-container');
    if (container) validateIPContainer(container);
}
    // ✅ YENİ FONKSİYON — initNtpPage dışına ekleyin
function setupNetworkFieldListeners() {
    const fields = [
        { prefix: 'subnet',  hiddenId: 'ntpSubnet'  },
        { prefix: 'gateway', hiddenId: 'ntpGateway' },
        { prefix: 'dns',     hiddenId: 'ntpDNS'     }
    ];
    
    fields.forEach(({ prefix, hiddenId }) => {
        for (let i = 1; i <= 4; i++) {
            const input = document.getElementById(`${prefix}-part${i}`);
            if (!input) {
                console.warn(`Input bulunamadı: ${prefix}-part${i}`);
                continue;
            }
            
            // Input event — değer değişince hidden input'u güncelle
            input.addEventListener('input', function() {
                // 255 sınırı
                if (parseInt(this.value) > 255) this.value = '255';
                
                // Otomatik sonraki alana geç
                if (this.value.length === 3 && i < 4) {
                    const next = document.getElementById(`${prefix}-part${i + 1}`);
                    if (next) { next.focus(); next.select(); }
                }
                
                // Hidden input'u güncelle
                updateNetworkHiddenInput(prefix, hiddenId);
            });
            
            // Keydown — nokta ve backspace navigasyonu
            input.addEventListener('keydown', function(e) {
                ...
                if (e.key === 'Backspace' && this.value === '' && i > 1) {
                    e.preventDefault();
                    const prev = document.getElementById(`${prefix}-part${i - 1}`);
    ...
    console.log('✅ Network field listener\'ları kuruldu');
}
...
function initNtpPage() {
    ...
    // ✅ YENİ: Subnet, Gateway, DNS için ayrıca event listener ekle
    setupNetworkFieldListeners();
...
function updateHiddenIPInputForField(fieldName) {
    // ID mapping — HTML'deki id'lerle birebir eşleşmeli
    const idMap = {
        'subnet':  'ntpSubnet',
        'gateway': 'ntpGateway',
        'dns':     'ntpDNS'     // ✅ Büyük harf
    };
    
    const hiddenId = idMap[fieldName];
    if (!hiddenId) {
        console.warn('Bilinmeyen field:', fieldName);
        return;
    }
    
    const hiddenInput = document.getElementById(hiddenId);
    if (!hiddenInput) {
        console.warn('Hidden input bulunamadı:', hiddenId);
        return;
    }
    ...
    hiddenInput.value = parts.join('.');
    console.log(`${fieldName} güncellendi:`, hiddenInput.value);
}
...
function loadIPToContainer(ip, containerSelector) {
    if (!ip || !validateIPFormat(ip)) return;
    
    const parts = ip.split('.');
    const container = document.querySelector(containerSelector);
    
    if (!container) {
        console.warn('Container bulunamadı:', containerSelector);
        return;
    }
...
function loadCurrentNTPToInputs(server1, server2, subnet, gateway, dns) {
    ...
    if (server2 && validateIPFormat(server2)) {
        ...
        for (let i = 1; i <= 4; i++) {
            const input = document.getElementById(`ntp2-part${i}`);
            if (input && parts[i-1]) input.value = parts[i-1];
        }
        updateHiddenIPInput(true);
    }
...
if (subnet && validateIPFormat(subnet)) {
    ...
    updateNetworkHiddenInput('subnet', 'ntpSubnet');   // ✅
}
...
if (gateway && validateIPFormat(gateway)) {
    ...
    updateNetworkHiddenInput('gateway', 'ntpGateway'); // ✅
}
...
if (dns && validateIPFormat(dns)) {
    ...
    updateNetworkHiddenInput('dns', 'ntpDNS');         // ✅
}
    
    // ✅ Tüm yüklemeler bittikten sonra hidden input'ları bir kez daha güncelle
    setTimeout(() => {
        updateHiddenIPInput(false);
        updateHiddenIPInput(true);
        updateHiddenIPInputForField('subnet');
        ...
        // Debug: Tüm hidden değerleri logla
        console.log('Hidden input kontrol:', {
            ntp1:    document.getElementById('ntpServer1')?.value,
            ntp2:    document.getElementById('ntpServer2')?.value,
            subnet:  document.getElementById('ntpSubnet')?.value,
            gateway: document.getElementById('ntpGateway')?.value,
            dns:     document.getElementById('ntpDNS')?.value,
        });
    }, 100);
}
